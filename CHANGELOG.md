...
emoteBuilds / Build.ListPublic(), so peer WNT/BLD sync never rebroadcasts them.
- Stats -> Recommendations: **Apply visible** applies every currently filtered recommendation in one click (respects Raise / Lower / Unapplied and the Echo vs Automation Logic tabs). Confirms when more than 20 rows are visible; toast reports how many applied. Per-row Apply / Dismiss / Inspect are unchanged. (Shipped as 3.86.5.)
- **Details_ProjectEbonhold 1.0.3-pe1:** Left-click a **PE Proc Sources** bar opens a Details-style attribution summary (damage, hits, average, other sources for that proc, other procs from that source). Custom Display script v4; hover tooltip shows the same stats.
...
- **Details_ProjectEbonhold 1.0.7-pe1:** Soft default overall_clear_newboss = false so Details **Overall Data** is not wiped on every new raid boss. One-shot migration for existing PE installs; re-enable wipe in Details options if desired. Docs: [details-project-ebonhold-pe.md](details-project-ebonhold-pe.md).

#### Tests
- 	ests/test_shared_loadout_bridge.lua covers mapping, cache isolation from peer sync, eligibility widen, and title dedupe vs peer builds.
- 	ests/test_select_dedup.lua and 	ests/test_soul_ashes.lua cover Autopilot Select dedup and Soul Ashes field fallbacks (#146).
