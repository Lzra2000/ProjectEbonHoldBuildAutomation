-- Details_TinyThreat.lua — Details! Tiny Threat plugin (WotLK 3.3.5a / Project Ebonhold).
-- Upstream: Details! Team v1.07; PE helpers in DetailsTinyThreatProjectEbonhold.lua.

local AceLocale = LibStub ("AceLocale-3.0")
local Loc = AceLocale:GetLocale ("Details_Threat")

local _GetNumSubgroupMembers = TT_GetNumSubgroupMembers
local _GetNumGroupMembers = TT_GetNumGroupMembers
local _UnitIsFriend = UnitIsFriend --> wow api
local _UnitName = UnitName --> wow api
local _UnitDetailedThreatSituation = TT_SafeUnitDetailedThreatSituation
local _IsInRaid = IsInRaid --> wow api
local _IsInGroup = IsInGroup --> wow api
local _UnitGroupRolesAssigned = TT_SafeUnitGroupRolesAssigned
local GetUnitName = TT_SafeGetUnitName
	...
	elseif (event == "ADDON_LOADED") then
		...
		if (AddonName == "Details_TinyThreat") then
			...
			if (TT_IsDetailsReady()) then
				...
				--> Install
				local install, saveddata = _G._detalhes:InstallPlugin ("RAID", Loc ["STRING_PLUGIN_NAME"], "Interface\\Icons\\Ability_Warrior_DefensiveStance", ThreatMeter, "DETAILS_PLUGIN_TINY_THREAT", MINIMAL_DETAILS_VERSION_REQUIRED, "Details! Team", "v1.07-pe1")
			...
			elseif (DEFAULT_CHAT_FRAME) then
				DEFAULT_CHAT_FRAME:AddMessage (
					"|cffFFAA00Details! Tiny Threat|r: Details! core add-on is required but not loaded.",
					1, 0.5, 0.5
				)
			end
		end
