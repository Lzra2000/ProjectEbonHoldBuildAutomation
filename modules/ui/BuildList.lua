...
local function CreateRow(parent)
    ...
    -- Locked echo icon buttons (18x18, bottom row). Sized to fit 6 slots
    -- inside the ~178px-wide left-column row -- at the old 22px/28px
    -- spacing the 6th icon (added when LOCKED_SLOTS went 5->6) overflowed
    -- the row by ~16px and got clipped by the scroll frame.
    row._lockedBtns = {}
...
local LOCKED_X_START = 28
local LOCKED_STEP    = 24
...
local function CreateTomeAtlasButton(parent)
    local btn = EbonBuilds.Theme.CreateButton(parent)
    btn:SetHeight(24)
    btn:SetText("Tome Atlas")
    btn:SetScript("OnClick", function()
...
local function CreateAffixesButton(parent)
    local btn = EbonBuilds.Theme.CreateButton(parent)
    btn:SetHeight(24)
    btn:SetText("Affixes")
    btn:SetScript("OnClick", function()
...
local function CreateNewBuildButton(parent, topAnchor)
    -- The one primary call-to-action in this stack: gold border sets it
    -- apart from the three peer navigation buttons above it.
    local btn = EbonBuilds.Theme.CreateButton(parent, "gold")
    btn:SetHeight(24)
...
function EbonBuilds.BuildList.Init(parent)
    ...
    local atlasBtn   = CreateTomeAtlasButton(parent)
    local affixBtn   = CreateAffixesButton(parent)
    newBuildBtn     = CreateNewBuildButton(parent, affixBtn)
    ...
    atlasBtn:SetPoint("TOPLEFT",   importBtn,       "BOTTOMLEFT",  0, -2)
    atlasBtn:SetPoint("TOPRIGHT",  parent,          "TOPRIGHT",    0,  0)
    affixBtn:SetPoint("TOPLEFT",   atlasBtn,        "BOTTOMLEFT",  0, -2)
    affixBtn:SetPoint("TOPRIGHT",  parent,          "TOPRIGHT",    0,  0)
