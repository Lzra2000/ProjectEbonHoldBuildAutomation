...
local function BuildCard(parent)
    ...
    item:SetBackdropColor(unpack(EbonBuilds.Theme.CARD_BG))
    item:SetBackdropBorderColor(unpack(EbonBuilds.Theme.BORDER_DIM))
...
function EbonBuilds.SessionHistory.RefreshSessionList()
    ...
    for i, s in ipairs(sorted) do
        ...
        -- Compare against the run before this one (sorted is newest-first).
        local prev = sorted[i + 1]
        local function DeltaTag(cur, old)
            if not prev or not cur or not old then return "" end
            local d = cur - old
            if d > 0 then return (" |cff44ff44+%d|r"):format(d) end
            if d < 0 then return (" |cffff4444%d|r"):format(d) end
            return " |cff888888=|r"
        end

        if isActive then
            item:SetBackdropBorderColor(0.27, 1.0, 0.27, 1)
            local lvl = s.maxLevel or UnitLevel("player")
            item._levelLabel:SetText(("|cff44ff44[Active]|r  Level %d%s"):format(lvl, DeltaTag(lvl, prev and prev.maxLevel)))
            ...
            local lvl = s.maxLevel or UnitLevel("player")
            item._levelLabel:SetText(("Level %d%s"):format(lvl, DeltaTag(lvl, prev and prev.maxLevel)))
        ...
        if isActive then
            item._soulLabel:SetText("Soul Ashes: ...")
        else
            item._soulLabel:SetText(("Soul Ashes: %s%s"):format(
                tostring(s.soulAshes), DeltaTag(s.soulAshes, prev and prev.soulAshes)))
        end
...
local function BuildLogRow(parent)
    ...
    for i = 1, 3 do
        ...
        -- Tooltip with score breakdown (weight + bonuses from the current
        -- active build; historical settings aren't logged, so this is the
        -- present-day math for the same echo).
        echoFrame:SetScript("OnEnter", function(self)
            if not self._echoName then return end
            ...
            local settings = build and build.settings
            if settings then
                -- Weights are keyed by the DB comment name, which differs
                -- from the spell name for class-specific echoes; resolve via
                -- spellId when the log entry has one (older logs may not).
                local wkey = self._echoSpellId
                    and EbonBuilds.Weights.CanonicalName(self._echoSpellId)
                    or self._echoName
                local w = EbonBuilds.Weights.Get(wkey) or 0
                GameTooltip:AddLine(" ")
                GameTooltip:AddLine("With current build settings:", 0.9, 0.82, 0.4)
                GameTooltip:AddLine(("  Base weight: %d"):format(w), 0.7, 0.7, 0.7)
                local q = self._echoQuality
                if q then
                    local qb = (settings.qualityBonus or {})[q]
                    if qb and qb ~= 0 then
                        local mode = (settings.qualityBonusMode or {})[q]
                        if mode then
                            GameTooltip:AddLine(("  Quality bonus: x%.1f"):format(qb), 0.7, 0.7, 0.7)
                        else
                            GameTooltip:AddLine(("  Quality bonus: +%d"):format(qb), 0.7, 0.7, 0.7)
                        end
                    end
                end
                -- Family bonuses for this echo's families
                local best = EbonBuilds.EchoTableRows.BuildBestByName and EbonBuilds.EchoTableRows.BuildBestByName()
                local dbEntry = best and best[self._echoName]
                if dbEntry and dbEntry.families then
                    for _, fam in ipairs(dbEntry.families) do
                        local fb = (settings.familyBonus or {})[fam] or (settings.familyBonus or {})[fam:gsub(" DPS", "")]
                        if fb and fb ~= 0 then
                            GameTooltip:AddLine(("  %s: +%d"):format(fam, fb), 0.7, 0.7, 0.7)
                        end
                    end
                end
            end
    ...
    for i, entry in ipairs(logs) do
        ...
        for j = 1, 3 do
            ...
            if ch then
                ...
                echoFrame._echoQuality = ch.quality
                echoFrame._echoSpellId = ch.spellId
