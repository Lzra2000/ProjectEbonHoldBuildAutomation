...
local stepLabel, backBtn, nextBtn, peakLabel

local state = {}
local echoListCache = {}

StaticPopupDialogs["EBONBUILDS_WIZARD_NO_ECHOES"] = {
    text = "You haven't rated any specific Echoes yet. Autopilot will rely only on rank and family modifiers, with no preference between individual Echoes. Continue anyway?",
    button1 = "Continue",
    button2 = "Go back",
    OnAccept = function()
...
------------------------------------------------------------------------
-- Settings preview -- shared by the live peak label and final creation,
-- so the two can never drift apart.
------------------------------------------------------------------------

local function BuildSettingsFromState()
    ...
    settings.noveltyValue = HasAdaptivePower() and (state.noveltyValue or 30) or 0

    return settings
end

-- Scores against the wizard's in-progress echo weights (not yet saved to
-- EbonBuilds.Weights), so the preview reflects choices made in Step 5
-- immediately, even before the build is created.
local function WizardWeightFn(name, quality)
    local e = state.echoes[name]
    return e and e.weight or 0
end

local function RefreshPeakPreview()
    if not peakLabel then return end
    local settings = BuildSettingsFromState()
    local name, score = EbonBuilds.Scoring.ComputePeak(state.class, settings, WizardWeightFn)
    if name then
        peakLabel:SetText(string.format("Current peak: |cffffd200%s|r = %d", name, score))
    else
        peakLabel:SetText("Current peak: -")
    end
end
...
------------------------------------------------------------------------
-- Class/Spec picker (Step 1 header) -- defaults to the current character,
-- but can be changed so you can plan a build for an alt or a different spec
-- ahead of time.
------------------------------------------------------------------------

local classSpecButtons = {}

local function RefreshClassSpecHighlight()
    for token, btn in pairs(classSpecButtons.class or {}) do
        if btn._border then
            if token == state.class then btn._border:Show() else btn._border:Hide() end
        end
    end
    for i, btn in ipairs(classSpecButtons.spec or {}) do
        if btn._border then
            if i == state.spec then btn._border:Show() else btn._border:Hide() end
        end
    end
end

local function BuildClassSpecRow(parent)
    classSpecButtons.class = {}
    classSpecButtons.spec  = {}

    local classLabel = parent:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    classLabel:SetPoint("TOPLEFT", parent, "TOPLEFT", 20, -14)
    classLabel:SetText("Class:")

    for i, token in ipairs(CLASS_ORDER) do
        local btn = CreateIconButton(parent, 22)
        local coords = CLASS_ICON_TCOORDS[token]
        btn._icon:SetTexture(CLASS_TEXTURE)
        if coords then btn._icon:SetTexCoord(coords[1], coords[2], coords[3], coords[4]) end
        btn:SetPoint("TOPLEFT", parent, "TOPLEFT", 60 + (i - 1) * 24, -12)
        ...
        btn:SetScript("OnEnter", function(self)
            ...
            GameTooltip:AddLine(token:sub(1,1) .. token:sub(2):lower())
            GameTooltip:Show()
        end)
        ...
        btn:SetScript("OnClick", function()
            ...
            state.class = token
            state.spec = 1
            RefreshClassSpecHighlight()
            RefreshPeakPreview()
        end)
        classSpecButtons.class[token] = btn
    end

    local specLabel = parent:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    specLabel:SetPoint("TOPLEFT", parent, "TOPLEFT", 20, -40)
    specLabel:SetText("Spec:")

    for i = 1, 3 do
        local btn = CreateIconButton(parent, 26)
        btn:SetPoint("TOPLEFT", parent, "TOPLEFT", 60 + (i - 1) * 30, -38)
        ...
        classSpecButtons.spec[i] = btn
    end

    -- Fill in spec icons/tooltips for whichever class is currently selected
    local specs = state.class and EbonBuilds.SpecData and EbonBuilds.SpecData[state.class]
    for i, btn in ipairs(classSpecButtons.spec) do
...
local function RenderStep1()
    ...
    local pickerRow = CreateFrame("Frame", nil, contentArea)
    pickerRow:SetPoint("TOPLEFT", contentArea, "TOPLEFT", 0, 0)
    pickerRow:SetPoint("TOPRIGHT", contentArea, "TOPRIGHT", 0, 0)
    pickerRow:SetHeight(60)
    BuildClassSpecRow(pickerRow)
    ...
    sub:SetText("Optional -- leave slots empty if you don't want any hard locks.")
    ...
    for i = 1, EbonBuilds.Build.LOCKED_SLOTS do
        ...
        btn:SetPoint("TOP", contentArea, "TOP", startX + (i - 1) * (slotSize + spacing), -160)
...
local function RenderStep2()
    ...
    local slider = CreateFrame("Slider", "EbonBuildsWizardNoveltySlider", contentArea)
    ...
    local low = contentArea:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall")
    low:SetPoint("TOPLEFT", slider, "BOTTOMLEFT", 0, -2)
    low:SetText("0")
    low:SetTextColor(unpack(EbonBuilds.Theme.TEXT_MUTED))

    local high = contentArea:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall")
    high:SetPoint("TOPRIGHT", slider, "BOTTOMRIGHT", 0, -2)
    high:SetText("100")
    high:SetTextColor(unpack(EbonBuilds.Theme.TEXT_MUTED))
...
local FAMILY_PRESETS = {
    { label = "Offense",  values = { Tank = 0,  Survivability = 0,  Healer = 0,  Caster = 20, Melee = 20, Ranged = 20 } },
    { label = "Balanced", values = { Tank = 10, Survivability = 10, Healer = 10, Caster = 10, Melee = 10, Ranged = 10 } },
    { label = "Survival", values = { Tank = 20, Survivability = 20, Healer = 10, Caster = 0,  Melee = 0,  Ranged = 0  } },
}
...
local function RenderStep3()
    ...
    local presetLabel = contentArea:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall")
    presetLabel:SetPoint("TOP", contentArea, "TOP", -140, -64)
    presetLabel:SetText("Quick presets:")

    for i, preset in ipairs(FAMILY_PRESETS) do
        local btn = EbonBuilds.Theme.CreateButton(contentArea)
        btn:SetWidth(90)
        btn:SetHeight(20)
        btn:SetPoint("TOP", contentArea, "TOP", -20 + (i - 1) * 95, -64)
        btn:SetText(preset.label)
        btn:SetScript("OnClick", function()
            for key, val in pairs(preset.values) do
                state.familyPriorities[key] = val
            end
            RenderStep3()
            RefreshPeakPreview()
        end)
    ...
    for i, entry in ipairs(FAMILIES) do
        RenderFamilyRow(entry, -104 - (i - 1) * 30)
    end
end
...
local function RenderQualityRow(q, anchorY)
    ...
    label:SetWidth(130)
    label:SetJustifyH("RIGHT")
    label:SetText("|cff" .. colorHex .. (QUALITY_LABELS[q] or tostring(q)) .. "|r")
...
local function RenderStep4()
    ...
    for i, q in ipairs(EbonBuilds.Quality.ORDER or {}) do
...
local function RenderEchoRow(parent, entry, index, y)
    ...
    local customBox -- forward-declared so button handlers can clear its highlight
    ...
    for j, opt in ipairs(WEIGHT_OPTIONS) do
        ...
        btn:SetScript("OnClick", function(self)
            ...
            if customBox then customBox:SetText("") end
            RefreshPeakPreview()
        end)
        btns[j] = btn
    end

    -- Custom exact-value input, for finer control than the 4 preset buttons.
    local customLabel = row:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall")
    customLabel:SetPoint("LEFT", row, "LEFT", btnStartX + 4 * 60 + 6, 0)
    customLabel:SetText("or:")

    customBox = CreateFrame("EditBox", nil, row, "InputBoxTemplate")
    customBox:SetWidth(34)
    customBox:SetHeight(18)
    customBox:SetPoint("LEFT", customLabel, "RIGHT", 4, 0)
    customBox:SetAutoFocus(false)
    customBox:SetNumeric(true)
    customBox:SetMaxLetters(3)
    local isPreset = false
    for _, opt in ipairs(WEIGHT_OPTIONS) do
        if opt.value == currentVal then isPreset = true end
    end
    if not isPreset then customBox:SetText(tostring(currentVal)) end

    local function ApplyCustomValue()
        local v = tonumber(customBox:GetText())
        if not v then return end
        v = math.max(0, math.min(100, math.floor(v)))
        state.echoes[entry.name].weight = v
        customBox:SetText(tostring(v))
        for _, b in ipairs(btns) do
            HighlightButton(b, b._val == v)
        end
        RefreshPeakPreview()
        customBox:ClearFocus()
    end
    customBox:SetScript("OnEnterPressed", ApplyCustomValue)
    customBox:SetScript("OnEditFocusLost", ApplyCustomValue)

    local removeBtn = EbonBuilds.Theme.CreateButton(row)
    removeBtn:SetWidth(56)
    removeBtn:SetHeight(18)
    removeBtn:SetPoint("LEFT", customBox, "RIGHT", 10, 0)
...
local function RenderStep5()
    ...
    local addBtn = EbonBuilds.Theme.CreateButton(contentArea)
    addBtn:SetWidth(120)
    addBtn:SetHeight(20)
    ...
    -- Themed scrollbar inset inside the picker backdrop
    local sb = EbonBuilds.Theme.CreateScrollBar(sf)
...
local function RenderStep0()
    ...
    -- Wizard button
    local wizardBtn = EbonBuilds.Theme.CreateButton(contentArea)
    ...
    local proBtn = EbonBuilds.Theme.CreateButton(contentArea)
    proBtn:SetWidth(200)
    proBtn:SetHeight(40)
...
local function GoNext()
    ...
    if state.step == 5 and next(state.echoes) == nil then
        StaticPopup_Show("EBONBUILDS_WIZARD_NO_ECHOES")
        return
    end
...
function view.Show(container, context)
    ...
    for _, quality in ipairs(EbonBuilds.Quality.ORDER or {}) do
        state.qualityBonus[quality] = quality * 10
    end
...
local function BuildViewFrame()
    ...
    -- Live peak-score preview (updates as quality/family/weight choices change)
    peakLabel = f:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    peakLabel:SetPoint("TOPRIGHT", f, "TOPRIGHT", -14, -12)
    peakLabel:SetText("")
    ...
    -- Navigation buttons
    backBtn = EbonBuilds.Theme.CreateButton(f)
    backBtn:SetWidth(80)
