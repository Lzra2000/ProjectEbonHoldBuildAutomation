...
local function ScoreChoice(choice, settings)
    ...
    local weight = EbonBuilds.Weights.Get(EbonBuilds.Weights.CanonicalName(spellId)) or 0
...
local function ScoreLockedEcho(lockedId, settings)
    ...
    local w = EbonBuilds.Weights.Get(EbonBuilds.Weights.CanonicalName(lockedId)) or 0
...
local function TrySelect(scored, settings, build)
    ...
    if #all == 0 then
        for _, s in ipairs(scored) do
            all[#all + 1] = s
            if not banList[s.spellId] then nonBanned[#nonBanned + 1] = s end
        end
    end
...
function EbonBuilds.Automation.Evaluate()
    ...
    local function body()
        ...
        if EbonBuilds.DebugLog.IsEnabled() then
            ...
            for _, s in ipairs(scored) do
                EbonBuilds.DebugLog.AddF("  [%d] %s q=%d score=%.0f w=%d%s%s%s",
                    ...
                    EbonBuilds.Weights.Get(EbonBuilds.Weights.CanonicalName(s.spellId)) or 0,
        ...
        if runData and (runData.remainingBanishes or 0) > 0 then
            ...
            for _, s in ipairs(scored) do
                if not s.isFrozen and not s.isCarried and not locallyFrozenIndices[s.index] and banList[s.spellId] then
