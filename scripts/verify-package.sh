...
if [ "$SKIP_DETAILS_SUITE" -eq 0 ] && [ -f dist/Details.zip ]; then
    ...
    if grep -q "1.0.4-pe1" "$DETAILS_UNPACK/Details_ProjectEbonhold/Details_ProjectEbonhold.toc" 2>/dev/null; then
        :
    else
        echo "PACKAGE FAIL: Details.zip Details_ProjectEbonhold.toc missing Version 1.0.4-pe1" >&2
