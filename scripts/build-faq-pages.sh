#!/usr/bin/env sh
# Generates modules/data/FAQContent.lua from docs/faq.md and
# CHANGELOG.md -- one page for "What's new" (the newest changelog
# entry) plus one page per FAQ question. Wired into release.sh, so the
# in-game FAQ can never drift from the shipped documents again (it had
# been showing 2.99 content at version 3.30 -- thirty releases of
# drift).
#
...
local md = ReadAll("docs/faq.md")

-- Newest changelog entry: first block whose line STARTS with "### " in
-- CHANGELOG.md. Line-anchored on purpose: an inline mention of "### x"
-- in prose (the file's own intro has one) must not match.
local changelog = ReadAll("CHANGELOG.md")
local firstHeader, firstBody
do
    local bodyLines
    for line in (changelog .. "\n"):gmatch("(.-)\n") do
        local h = line:match("^### (.+)$")
        if h then
            if firstHeader then break end
            firstHeader, bodyLines = h, {}
        elseif bodyLines then
...
local currentQ, currentBody
local function FlushQ()
    if currentQ then
        local a = table.concat(currentBody, "\n"):gsub("^%s+", ""):gsub("%s+$", "")
        if currentQ ~= "" and a ~= "" then
            qas[#qas + 1] = { q = currentQ, a = a, category = currentCategory }
        end
    end
    currentQ, currentBody = nil, nil
end
for line in (faqSection .. "\n"):gmatch("(.-)\n") do
    local q = line:match("^### (.+)$")
    local c = (not q) and line:match("^## (.+)$") or nil
    if q then
        FlushQ()
        currentQ, currentBody = q, {}
    elseif c then
        FlushQ()
        currentCategory = c
...
emit("local addonName, EbonBuilds = ...")
emit("")
emit("-- EbonBuilds: modules/data/FAQContent.lua")
emit("-- GENERATED by scripts/build-faq-pages.sh from docs/faq.md and")
emit("-- CHANGELOG.md -- do not edit by hand; edit those and re-run the")
emit("-- script (release.sh does).")
emit("")
